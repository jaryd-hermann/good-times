-- Migration: Add Comment Thread Notifications
-- When a user comments on a post, notify all previous commenters (including the entry author)
-- This keeps users engaged in discussion threads they've joined

-- Update: queue_new_comment_notification function
-- Now notifies both:
-- 1. The entry author (if they didn't write the comment)
-- 2. All previous commenters on the same entry (if they didn't write the comment)
...
-- Note: The trigger already exists (on_new_comment), so it will automatically use the updated function
