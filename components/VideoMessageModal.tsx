...
export function VideoMessageModal({ visible, question, onClose, onAddVideo }: VideoMessageModalProps) {
  ...
  const theme2Colors = useMemo(() => {
    if (isDark) {
      return {
        ...
        pink: "#D97393", // Pink for record button
      }
    } else {
      return {
        red: "#B94444",
  ...
  async function startRecording() {
    ...
    try {
      ...
      // Start recording with expo-camera
      // Note: expo-camera uses default quality settings (not 4K)
      ...
      // Store the promise - we'll handle it when recording stops
      recordingRef.current = {
        stop: async () => {
          // Stop recording by calling stopRecording on the camera
          if (cameraRef.current) {
      ...
      // Start timer to track duration and file size
      ...
      recordingPromise
        .then(async (result) => {
          // Recording completed - result contains the URI
          if (timerRef.current) {
  ...
  async function pauseRecording() {
    ...
    try {
      // expo-camera doesn't support pause/resume, so we'll stop and restart
      // For now, we'll just stop the recording
      stopRecording()
    } catch (error: any) {
  ...
  // Resume recording - expo-camera doesn't support resume, so start new recording
  async function resumeRecording() {
    // expo-camera doesn't support resume, so we can't implement this
    // Instead, we'll just start a new recording
    startRecording()
  }
  ...
  async function stopRecording() {
    ...
    try {
      ...
      // The recording promise will handle setting the URI when it completes
      // We'll wait for it in the startRecording promise handler
    } catch (error: any) {
  ...
  // Handle scrub (seek)
  async function handleScrub(position: number) {
    if (!videoRef.current) return
    ...
    try {
      ...
      setError(error.message || "Failed to seek video")
    }
  }
  ...
  // Add video to entry
  async function handleAddToEntry() {
    if (!videoUri) return
  ...
  function handleClose() {
    // Only show discard confirmation if there's a video that hasn't been added to entry
  ...
  const styles = useMemo(() => StyleSheet.create({
    ...
    cameraCard: {
      width: "100%",
      aspectRatio: 9 / 16, // Portrait orientation (taller)
      maxHeight: Dimensions.get("window").height * 0.6, // Limit height to 60% of screen to ensure CTAs are visible
      borderRadius: 16,
      overflow: "hidden",
      ...
      <View style={styles.modal}>
        <View style={styles.container}>
          ...
          <View style={styles.contentWrapper}>
            ...
            {/* Action Buttons - Delete and Add to Entry (shown in playback state) */}
            {recordingState === "playback" && videoUri && (
              <View style={styles.actionButtons}>
                <TouchableOpacity style={[styles.actionButton, styles.deleteButton]} onPress={deleteVideo}>
                  <Text style={styles.actionButtonText}>Delete</Text>
                </TouchableOpacity>
                <TouchableOpacity style={[styles.actionButton, styles.addButton]} onPress={handleAddToEntry}>
                  <Text style={styles.addButtonText}>Add to Entry</Text>
                </TouchableOpacity>
              </View>
            )}
          </View>
                ...
                <TouchableOpacity
                  ...
                  onPress={() => {
                    if (recordingState === "idle") {
                      ...
                      // Clicking record button stops recording
                      stopRecording()
                    }
                  }}
                >
